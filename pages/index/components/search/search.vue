<template>
	<uni-transition :modeClass="['slide-left']" :styles="tranStyles" :show="show">
		<view class="d-flex flex-column overflow-hidden">
			<view class="search-box">
				<view class="search-input">
					...
					<input type="text" v-model="keyword" placeholder="搜个芝字试试看" placeholder-class="placeholder"
						@input="handleKeywordInput" />
					<image src="/static/images/common/image-delete.png" class="close-icon" @tap="clear"></image>
				</view>
			...
			<scroll-view scroll-y class="result">
				<template v-if="!result.length">
					<view class="section">
						...
						<view class="list">
							<view class="item" v-for="(item, index) in historySearch" :key="index"
					...
					<view class="section">
						...
						<view class="list">
							<view class="item" v-for="(item, index) in hotSearch" :key="index"
								...
								<image :src="item.nameImage" v-if="item.nameImage" class="name-image"></image>
							</view>
						</view>
					</view>
				</template>
					...
					<view class="wrapper" style="padding: 0 30rpx 30rpx 0;">
						<view class="product" v-for="(item, index) in result" :key="index"
							...
							<view class="d-flex align-items-center">
								<image :src="item.images[0].url" class="pro-image"></image>
		...
		methods: {
			...
			handleChoose(item, isSearch = false) {
				...
				this.categories.forEach((category) => {
					const find = category.product.find((product) => product.id == item.productId);
					if (find) {
						this.hide();
